...
The account security log from `account.listLogs()`, when the server exposes it. Appwrite Cloud does. Self-hosted Appwrite 2.x removed the endpoint, and in that case the tab hides itself after the first failed request.
