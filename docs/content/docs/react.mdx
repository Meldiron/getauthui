...
<>
  ...
  <button onClick={() => setModalOpen(false)}>Force close</button>
  <AuthUIModal open={modalOpen} />
</>
```
...
Prefer `authui-show:not([ready])` over `:defined` for FOUC CSS. React can define custom elements before Auth UI sets `ready`, so `[ready]` is the reliable gate. See [`<authui-show>`](/docs/components/show).
