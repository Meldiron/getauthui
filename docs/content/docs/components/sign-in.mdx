...
While the meter is visible, Auth UI also checks Have I Been Pwned with the [range k-anonymity API](https://haveibeenpwned.com/API/v3#PwnedPasswords) (only the first 5 characters of the SHA-1 hash leave the browser). Breached passwords show a warning and block submit. If the check cannot reach the network, the form stays usable and Appwrite can still reject with `password_pwned`.
