...
| `view`             | `"sign-in"` / `"sign-up"` / `"forgot-password"` / `"magic-url"` / `"email-otp"` / `"phone"` / `"account"` | `"sign-in"` | Screen to show.                                                                               |
| `close-on-success` | `boolean`                                                                                                 | `true`      | Close after a successful sign in. Ignored when `successUrl` is set, since the page navigates. |
...
- Switches to the account screen when a signed-in user asks for "Manage account" from the inline sign-in panel.
