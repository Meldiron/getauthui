...
- **CDN users:** put this critical CSS in your page `<head>` so protected content stays hidden before the module executes (same string as the exported `CRITICAL_FOUC_CSS` from `@getauthui/core`):
