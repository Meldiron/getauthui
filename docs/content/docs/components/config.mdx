...
- **Wrong project ID or unregistered hostname.** The first `account.get()` returns `project_not_found` or `general_unknown_origin`. Auth UI emits an `error` event, logs a console warning, and shows the same banner on `<authui-sign-in>`, `<authui-button>`, and the signed-out state of `<authui-user-button>` (so guestbook / launcher chrome surfaces it without opening the modal).
