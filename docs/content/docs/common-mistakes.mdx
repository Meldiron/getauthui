...
| Script                                                            | What you get                                                     |
| ----------------------------------------------------------------- | ---------------------------------------------------------------- |
| `https://unpkg.com/@getauthui/core@0.1.20` (ESM, `type="module"`) | Custom elements are defined. No `window.AuthUI`.                 |
...
The in-module FOUC guard cannot run until the script loads. Put this critical CSS in `<head>` before the Auth UI script so `<authui-show>` children stay hidden (exported as `CRITICAL_FOUC_CSS` from `@getauthui/core`):
