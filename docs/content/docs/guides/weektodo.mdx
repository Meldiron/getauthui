...
Open the URL Vue CLI prints (usually http://localhost:8080). You get the full planner in the browser. Prefer this web build for Auth UI. Electron notes are at the end.
...
```bash
yarn add @getauthui/core@0.1.8 appwrite lit
```
...
app.config.compilerOptions.isCustomElement = (tag) => tag.startsWith("authui-");
app.use(store);
app.use(i18n);
app.mount("#app");
```

`import "@getauthui/core"` registers the custom elements. `isCustomElement` tells Vue 3 to leave `authui-*` tags alone so Lit owns them. `AuthUI.init` configures the shared store. Replace the endpoint and project ID with yours.
...
export async function downloadBackup() {
  ...
  return AuthUI.getClient().call("get", url); // JSON file, client parses it
}
```
...
<authui-show when="signed-out">
  <div class="form-check form-switch d-flex px-1 mb-3 justify-content-between align-items-center">
    ...
    <button id="sign-in-sync-btn" type="button" class="btn py-1 px-2 border" style="width: 140px;"
      @click="openSignIn">
...
<authui-show when="signed-in">
  <div class="form-check form-switch d-flex px-1 mb-3 justify-content-between align-items-center">
    ...
    <button id="backup-data-btn" type="button" class="btn py-1 px-2 border" style="width: 140px;"
      :disabled="busy" @click="backUp">
  ...
  <div class="form-check form-switch d-flex px-1 mb-3 justify-content-between align-items-center">
    ...
    <button id="restore-data-btn" type="button" class="btn py-1 px-2 border" style="width: 140px;"
      :disabled="busy" @click="restore">
  ...
  <div class="form-check form-switch d-flex px-1 mb-3 justify-content-between align-items-center">
    ...
    <button id="account-btn" type="button" class="btn py-1 px-2 border" style="width: 140px;"
      @click="openAccount">
...
If you want a persistent trigger outside Settings, drop a user button at the bottom of `src/components/layout/sideBar.vue`, above the gear icon:

```html title="src/components/layout/sideBar.vue"
<authui-user-button style="margin: 0.5rem auto"></authui-user-button>
```

[`<authui-user-button>`](/docs/components/user-button) shows Sign in while signed out and the avatar menu while signed in. No extra wiring.
