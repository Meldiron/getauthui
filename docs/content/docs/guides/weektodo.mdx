...
<authui-user-button style="width: 40px; margin: 0.5rem auto; overflow: hidden"></authui-user-button>
```

[`<authui-user-button>`](/docs/components/user-button) shows Sign in while signed out and the avatar menu while signed in; WeekToDo’s sidebar is an icon-only rail about 58px wide, so the default Sign in pill overflows unless the button is constrained as shown.
