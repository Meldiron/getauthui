...
```space-lua
-- priority: 50
local AuthUI = js.import("https://unpkg.com/@getauthui/core@0.1.10").AuthUI
...
- **Load it server-side instead.** In single-space mode the `SB_HEAD_HTML` environment variable injects raw HTML into `<head>`, so `<script type="module" src="https://unpkg.com/@getauthui/core@0.1.10">` can go there and `js.window.AuthUI` becomes available without `js.import`. It is not in SilverBullet's docs but it works.
